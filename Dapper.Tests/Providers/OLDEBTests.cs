    ...
    public class OLDEBTests : TestBase<OLEDBProvider>
    {
        ...
        public void PseudoPositionalParameters_ExecSingle()
        {
            using (var connection = GetOleDbConnection())
            {
                ...
                long count = connection.Execute("insert #named_single (val) values (?x?)", data);
                long sum = (long)connection.ExecuteScalar("select sum(val) from #named_single");
        ...
        public void PseudoPositionalParameters_ExecMulti()
        {
            using (var connection = GetOleDbConnection())
            {
                ...
                long count = connection.Execute("insert #named_multi (val) values (?x?)", data);
                long sum = (long)connection.ExecuteScalar("select sum(val) from #named_multi");
